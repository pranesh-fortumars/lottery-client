...
const AdminControl = () => {
  ...
  return (
    <div className="space-y-10 pb-32 p-4 min-h-screen bg-[#f8f9fa]">
      ...
      <div className="border-[1.5px] border-[#4f46e5] rounded-[2.5rem] p-8 bg-gray-900 text-white shadow-2xl relative overflow-hidden group">
         <div className="absolute top-0 right-0 w-32 h-32 bg-[#4f46e5]/20 rounded-full blur-3xl"></div>
         <div className="flex gap-4 items-center mb-8">
            ...
            <div className="flex-grow">
               ...
               <p className="text-[#4f46e5] font-black text-[10px] uppercase tracking-widest leading-none mt-1">Live Draw Management</p>
            </div>
         </div>
         ...
         <div className="flex gap-4 justify-between bg-white/5 p-5 rounded-2xl border border-white/5">
            <div className="flex items-center gap-3">
               ...
               <span className="text-[10px] font-black uppercase tracking-widest text-[#6366f1]">Active Node Proxy</span>
            </div>
      ...
      <div className="space-y-10">
        ...
        <div className="space-y-6">
           <div className="flex items-center gap-3 ml-2">
              <Gamepad2 className="text-[#6366f1]" size={20} />
           ...
           <div className="space-y-4">
              {activeLotteries.map((item) => (
                <div key={item.id} className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-lg flex items-center justify-between group active:scale-[0.98] transition-all hover:border-[#6366f1]/20 relative overflow-hidden">
                   <div className="flex items-center gap-5 relative z-10">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg border border-white shadow-sm ${
                        item.status === 'Running' ? 'bg-[#eff6ff] text-[#6366f1]' : 'bg-gray-50 text-gray-300'
                      }`}>
                      ...
                      <div>
                         ...
                         <div className="flex items-center gap-2 mt-1">
                            <span className="text-[9px] text-[#6366f1] font-black uppercase tracking-widest">{item.status}</span>
        ...
        <div className="space-y-6 pt-4 border-t border-gray-100">
           ...
           <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-xl space-y-8">
              <div className="grid grid-cols-1 gap-6">
                {[
                  ...
                  <div key={idx} className="space-y-2 group/param">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2 block group-focus-within/param:text-[#6366f1] transition-colors">{param.label}</label>
                    <div className="flex items-center bg-gray-50/50 border border-gray-100 rounded-2xl px-6 h-16 group-focus-within/param:border-[#6366f1]/20 group-focus-within/param:bg-white transition-all shadow-sm">
                       <param.icon className="text-[#6366f1] opacity-20 mr-4" size={24} />
                       <input 
                         type="text" 
                         inputMode="numeric"
                         pattern="[0-9]*"
              ...
              <button className="w-full h-16 bg-gray-900 text-white rounded-2xl font-black text-[12px] uppercase tracking-widest shadow-xl flex items-center justify-center gap-3 active:scale-95 transition-all shadow-black/10">
                 Apply Global Safeguards <ChevronRight size={20} className="text-[#6366f1]" />
              </button>
           </div>
        </div>
      </div>
